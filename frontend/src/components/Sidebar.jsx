...
const PANELS = [
  { key: 'descarga',  icon: 'download',      cls: 'blue',  label: 'Descargar datos',    sub: 'Informe PDF y CSV' },
  { key: 'sala',      icon: 'meeting_room',  cls: 'green', label: 'Condiciones de sala', sub: 'Configurar aula' },
  { key: 'historial', icon: 'notifications', cls: 'red',   label: 'Historial de alertas', sub: 'Eventos fuera de rango' },
  { key: 'nosotros',  icon: 'groups',        cls: 'gray',  label: 'Sobre nosotros',       sub: 'El equipo Monitoriza' },
]
      ...
      <div className={`${styles.sidebar} ${open ? styles.sidebarOpen : ''}`}>
        <div className={styles.sbTop}>
          ...
          <div style={{ padding: '0 1.5rem 1rem 1.5rem' }}>
            <button
              ...
              style={{
                ...
                color: soundEnabled ? 'white' : '#475569',
                cursor: 'pointer',
                fontWeight: 'bold',
                display: 'flex',
                alignItems: 'center',
            ...
            >
              <Icon
                name={soundEnabled ? 'notifications_off' : 'notifications_active'}
          ...
          {PANELS.map(p => (
            <div
              key={p.key}
              className={`${styles.menuItem} ${activePanel === p.key ? styles.menuActive : ''}`}
              onClick={() => setActivePanel(p.key)}
            >
              <div className={`${styles.menuIcon} ${styles[p.cls]}`}>
                <Icon name={p.icon} />
              </div>

              <div>
                <div className={styles.menuLabel}>{p.label}</div>
                <div className={styles.menuSub}>{p.sub}</div>
              </div>
            </div>
          ))}
            ...
            <div className={styles.panel}>
              ...
              <div className={styles.exportCard}>
                <div className={styles.exportCardHeader}>
                  <Icon name="description" className={styles.exportIcon} />
              ...
              <div className={styles.exportCard}>
                <div className={styles.exportCardHeader}>
                  <Icon name="table_chart" className={styles.exportIcon} />
              ...
              {!conditionsLoading && !conditionsError && conditions && (
                <>
                  ...
                  <input
                    ...
                    type="number"
                    min="0"
                    value={conditions.windows !== undefined ? conditions.windows : ''}
                  ...
                  <select
                    className={styles.formInput}
                    value={conditions.cortinas || 'no'}
                    onChange={e => setConditions(c => ({ ...c, cortinas: e.target.value }))}
                  >
                ...
                <>
                  {activeAlerts && activeAlerts.map(a => (
                    ...
                    >
                        ...
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                          ...
                          <div className={styles.alertInfo}>
                            ...
                            <div className={styles.alertMsg}>
                              {a.high ? 'Por encima' : 'Por debajo'} del rango — <strong>{a.value} {a.unit}</strong>
                            </div>
                          </div>
                        </div>

                        <div className={styles.alertTime} style={{ color: 'var(--warn)', fontWeight: 'bold' }}>
                          LIVE
                        </div>
                      </div>
                      ...
                      >
                        <Icon name="lightbulb" />
                        <span>{getContextualRecommendation(a, conditions)}</span>
                      </div>
                    </div>
                  ))}
                  ...
                  {alerts.slice(0, 15).map(a => (
                    <div key={a.id} className={styles.alertItem}>
                      ...
                      <div className={styles.alertInfo}>
                        ...
                        <div className={styles.alertMsg}>
                          {a.high ? 'Por encima' : 'Por debajo'} del rango — {a.value} {a.unit}
                        </div>
                      </div>
            ...
            <div className={styles.panel}>
              ...
              <div className={styles.aboutCard}>
                ...
                <div className={styles.aboutDesc}>
                  Sistema IoT basado en ESP8266 para optimizar las condiciones de aprendizaje en tiempo real.
                </div>
              </div>
              ...
              <div className={styles.aboutCard}>
                ...
                <div className={styles.aboutDesc}>
                  Frontend React + Vite. Backend ESP8266. IA para recomendaciones contextualizadas.
                </div>
              </div>
        ...
        <div className={styles.catZone}>
          <button className={styles.catBtn} onClick={handleCatClick}>
            <Icon name="pets" className={styles.catPaw} />
        ...
        <div className={styles.catModal} onClick={() => setCatOpen(false)}>
          <div className={styles.catCard} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.catDesc}>
              Guardián oficial de mi-aula.
              <br />
              Monitorea que todo esté bien desde su rincón favorito.
            </div>

            <button className={styles.catClose} onClick={() => setCatOpen(false)}>
              hasta luego, Byte
            </button>
          </div>
        </div>
